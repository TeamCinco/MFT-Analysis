...
void FastCSVWriter::write_ohlcv_with_features(
    ...
    for (size_t i = 0; i < ohlcv_data.size(); ++i) {
        ...
        // Technical indicators - use safe_get_value to handle different vector sizes
        file << safe_get_value(returns, i > 0 ? i - 1 : 0) << ",";  // returns start from index 1
        ...
        file << safe_get_value(velocity, i > 0 ? i - 1 : 0) << ",";  // velocity starts from index 1
        file << safe_get_value(acceleration, i > 1 ? i - 2 : 0) << ",";  // acceleration starts from index 2
